...
func burnCTAUT(identifier abelian.AutId) {
	...
	joyAbelAddress, err := pseudonymCTAccount.GenerateAbelAddress()
	if err != nil {
	...
	recipients := make([]*abelian.Recipient, 0, len(tmpAddresses)+1)
	receiverAddresses := make([][]byte, 0, len(tmpAddresses)+1)
	...
	// for burning, append a public recipient
	...
	if selectedValue-targetTokenValue != 0 {
		...
		if recipient.HideValue {
			...
			// append before burn-token
			recipients = slices.Insert(recipients, len(recipients)-1, recipient)
			receiverAddresses = slices.Insert(receiverAddresses, len(receiverAddresses)-1, changeAddress)
		}
	}
